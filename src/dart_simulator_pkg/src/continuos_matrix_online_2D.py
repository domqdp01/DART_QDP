...
def continuous_matrices_2D(z, u):
    """
    ...
    F_y_f_0 = float(F_y_f_0)  # Estrae il valore scalare dall'array
    F_y_r_0 = float(F_y_r_0)
    u_scalar = float(u[1])
    ...
    # G_0 = np.array([
    #             [-1/m * F_y_f_0 * np.sin(u[1]), 0],
    #             [1/m * F_y_f_0 * np.cos(u[1]), 1/m * F_y_r_0],
    #             [1/Jz * (lf * F_y_f_0 * np.cos(u[1])), -1/Jz * lr * F_y_r_0]
    #         ])
    F_0 = np.array([
    1/m * (F_x_0_r + F_x_0_f * np.cos(u_scalar)) + z[2] * z[1],
    1/m * (F_x_0_f * np.sin(u_scalar)) - z[2] * z[0],
    lf/Jz * (F_x_0_f * np.sin(u_scalar))
    ]).reshape(-1, 1)

    G_0 = np.array([
        [-1/m * F_y_f_0 * np.sin(u_scalar), 0],
        [1/m * F_y_f_0 * np.cos(u_scalar), 1/m * F_y_r_0],
        [1/Jz * (lf * F_y_f_0 * np.cos(u_scalar)), -1/Jz * lr * F_y_r_0]
    ])
